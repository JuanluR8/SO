...
*
* @param path file path
* @return result=0 if success, result<0 on error
*/
static int my_unlink(const char *path) {
  ...
  if(dirPosition != -1){
    // Busco el i-nodo en el que se encuentra el archivo
		int idxNode = myFileSystem.directory.files[dirPosition].nodeIdx;
    // Pongo el tamaño del i-nodo a 0
		resizeNode(idxNode, 0);
    // Marco el i-nodo como libre
		myFileSystem.nodes[idxNode]->freeNode = true;
    // Pongo la direccion como libre
		myFileSystem.directory.files[dirPosition].freeFile = true;
    // Actualizo el numero de i-nodos libres
		myFileSystem.numFreeNodes++;
    // Actualizo el numero de ficheros que tiene mi sistema
		myFileSystem.directory.numFiles--; 

    //Actualizo el mapa de bits
		int i;
...
struct fuse_operations myFS_operations = {
    ...
    .read     = my_read             // Reads a file content
};
